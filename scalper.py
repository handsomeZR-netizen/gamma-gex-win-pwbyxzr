...
if os.getenv("GAMMA_ENABLE_LEGACY_TRADING", "0") != "1":
    print("scalper.py is disabled in this build. Trading execution has been retired.")
    print("Use: python run_dashboard.py")
    sys.exit(1)

from app.common.filelock import lock_stream, unlock_stream
from app.common.paths import (
    GAMMA_HOME,
    YFINANCE_WARNINGS_LOG,
    TRADES_FILE,
    ACCOUNT_BALANCE_FILE as DEFAULT_ACCOUNT_BALANCE_FILE,
    ORDERS_LIVE_FILE,
    ORDERS_PAPER_FILE,
    tmp_path,
)
...
yfinance_handler = logging.FileHandler(str(YFINANCE_WARNINGS_LOG))
...
LOCK_FILE = str(tmp_path(f"gexscalper_{INDEX_CONFIG.code.lower()}_{mode.lower()}.lock"))
...
try:
    ...
    lock_stream(lock_fd, exclusive=True, blocking=False)
...
ACCOUNT_BALANCE_FILE = str(DEFAULT_ACCOUNT_BALANCE_FILE)  # Track balance across restarts
...
try:
    ...
    if setup['strategy'] == 'SKIP':
        ...
        try:
            sys.path.insert(0, str(GAMMA_HOME))
    ...
    ORDERS_FILE = str(ORDERS_PAPER_FILE) if mode == "PAPER" else str(ORDERS_LIVE_FILE)
