...
if os.getenv("GAMMA_ENABLE_LEGACY_TRADING", "0") != "1":
    print("monitor.py is disabled in this build. Trading monitor has been retired.")
...
# File paths
ORDERS_FILE_PAPER = str(ORDERS_PAPER_FILE)
ORDERS_FILE_LIVE = str(ORDERS_LIVE_FILE)
TRADE_LOG_FILE = str(TRADES_FILE)
LOG_FILE_PAPER = str(MONITOR_PAPER_LOG)
LOG_FILE_LIVE = str(MONITOR_LIVE_LOG)
...
def get_mode():
    """
    Detect trading mode from command line args, env var, then local sentinel file.
    """
    # PRIORITY 1: Explicit command line argument (highest priority)
    if len(sys.argv) > 1:
        arg = sys.argv[1].upper()
        if arg in ['LIVE', 'REAL']:
            print('Gamma Trading Mode: LIVE MODE (explicit arg)')
            return 'REAL'
        if arg in ['PAPER', 'DEMO']:
            print('Gamma Trading Mode: PAPER MODE (explicit arg)')
            return 'PAPER'

    # PRIORITY 2: Environment variable
    env_mode = os.getenv('GAMMA_TRADING_MODE', '').strip().upper()
    if env_mode in {'LIVE', 'REAL'}:
        print('Gamma Trading Mode: LIVE MODE (env)')
        return 'REAL'
    if env_mode in {'PAPER', 'DEMO'}:
        print('Gamma Trading Mode: PAPER MODE (env)')
        return 'PAPER'

    # PRIORITY 3: Local sentinel file in project root
    sentinel_path = Path(GAMMA_HOME) / 'GAMMA_TRADING_MODE'
    if sentinel_path.exists():
        try:
            sentinel_mode = sentinel_path.read_text(encoding='utf-8').strip().upper()
            if sentinel_mode in {'LIVE', 'REAL'}:
                print('Gamma Trading Mode: LIVE MODE (sentinel)')
                return 'REAL'
            if sentinel_mode in {'PAPER', 'DEMO'}:
                print('Gamma Trading Mode: PAPER MODE (sentinel)')
                return 'PAPER'
        except Exception as e:
            print(f'WARNING: Could not read GAMMA_TRADING_MODE sentinel: {e}')

    # PRIORITY 4: Default to PAPER (safest)
    print('Gamma Trading Mode: PAPER MODE (default)')
    return 'PAPER'
...
if MODE == 'REAL':
    DISCORD_STORAGE_FILE = str(data_path('discord_messages_live.json'))
    DISCORD_WEBHOOK_URL = DISCORD_WEBHOOK_LIVE_URL
    HEALTHCHECK_URL = HEALTHCHECK_LIVE_URL
else:
    DISCORD_STORAGE_FILE = str(data_path('discord_messages_paper.json'))
...
def load_orders():
    ...
    if not os.path.exists(ORDERS_FILE):
        return []
    try:
        with locked_open(ORDERS_FILE, 'r', exclusive=False) as f:
...
def save_orders(orders):
    ...
    with locked_open(ORDERS_FILE, 'w', exclusive=True) as f:
    ...
    try:
        ...
        for row in rows[1:]:  # Skip header
            if row and len(row) > 1 and row[1] == str(order_id):
                ...
                try:
                    balance_file = str(data_path('account_balance.json'))
