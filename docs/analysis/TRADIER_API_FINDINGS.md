...
```bash
TRADIER_SANDBOX_KEY=<TRADIER_SANDBOX_KEY>
TRADIER_LIVE_KEY=<TRADIER_LIVE_KEY>
```
...
```bash
curl -H "Authorization: Bearer <TRADIER_LIVE_KEY>" \
...
```bash
export TRADIER_API_TOKEN="<TRADIER_LIVE_KEY>"
...
# Add:
0 17 * * 1-5 cd /gamma-scalper && export TRADIER_API_TOKEN="<TRADIER_LIVE_KEY>" && python3 collect_spy_qqq_options.py >> data_collector.log 2>&1
```
